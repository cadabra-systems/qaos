...
namespace Qaos {
	...
	public:
		...
		static inline bool IsInteger(const QString& s)
		{
			if (s.isEmpty()) {
				return false;
			}
				...
				return (s.length() > 1) &&
					   (std::find_if(s.begin() + 1, s.end(), [](QChar c) { return !c.isDigit(); }) == s.end());
			}
			else {
				return std::find_if(s.begin(), s.end(), [](QChar c) { return !c.isDigit(); }) == s.end();
			}
		}
	/** @} */
